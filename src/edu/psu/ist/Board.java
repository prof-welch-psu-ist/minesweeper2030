...
public record Board(Vector<Row> rows) {
    ...
    public TileType type(int row, int col) {
     ...
     * building instances of records... Here we can't mark the constructor
     * of a record private (as we could with a normal class via Bloch's builder pattern).
     * <p>
     * It is unfortunate that records always must have a public constructor (at least in
     * terms of this older design pattern). Now someone can conceivably construct a non-square board object like:
     * <pre><code>
